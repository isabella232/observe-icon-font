...
module.exports = function getSvgFiles (config = defaultConfig, destinationPath) {
  return Promise.all([
    ...
    getiDFiles(config.iD, destinationPath)
  ])
}

function getiDFiles (options, destinationPath) {
  if (!options) return

  const packagePath = path.join(__dirname, '..', 'iD')
  const iconsPath = path.join(packagePath, 'svg', 'iD-sprite', 'presets')
